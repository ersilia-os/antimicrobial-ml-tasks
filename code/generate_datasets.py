...
class TypeDatasets():
    ...
    def run_type(self, df):
        df = df[df.target_type == 'ORGANISM']
        top_types = self.top_type_selector(df)
        types_data = {}
        for t in top_types:
            df_ = df[df.standard_type==t]
            lc, hc = self.create_datasets(df_)
            types_data[t] = lc, hc
        return types_data


class AssayDatasets(TypeDatasets):
    def __init__(self, pathogen):
        self.pathogen = pathogen
